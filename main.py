...
# --- ГЕНЕРАТОР КАРТИНОК (НОВЫЙ ДИЗАЙН) ---
async def create_banner(member, title_text, bg_filename):
    ...
    # Шрифты
    try:
        font_large = ImageFont.truetype("font.ttf", 90) # Увеличили
    ...
    # 1. Заголовок (WELCOME / GOODBYE)
    # Вычисляем размер текста, чтобы найти центр
    _, _, w_title, h_title = draw.textbbox((0, 0), title_text, font=font_large)
    x_title = (W - w_title) / 2
    y_title = H / 2 - 80 # Чуть выше центра

    # 2. Имя пользователя
    user_text = f"{member.name}"
    _, _, w_user, h_user = draw.textbbox((0, 0), user_text, font=font_small)
    x_user = (W - w_user) / 2
    y_user = H / 2 + 30 # Чуть ниже центра

    # Рисуем с ТЕНЬЮ (для красоты)
    shadow_offset = 4
    draw.text((x_title + shadow_offset, y_title + shadow_offset), title_text, fill="black", font=font_large)
    draw.text((x_user + shadow_offset, y_user + shadow_offset), user_text, fill="black", font=font_small)

    # Основной текст
    draw.text((x_title, y_title), title_text, fill=(255, 255, 255), font=font_large)
    draw.text((x_user, y_user), user_text, fill=(0, 255, 255), font=font_small) # Cyan цвет
...
# --- ПРИВАТНЫЕ ВОЙСЫ (ПРАВА БОТА ИСПРАВЛЕНЫ) ---
...
class PrivateVoiceCreateModal(discord.ui.Modal, title='Создать комнату'):
    ...
    async def on_submit(self, i):
        try:
            ...
            # ПРАВА: БОТ ПОЛУЧАЕТ ПОЛНЫЙ ДОСТУП АВТОМАТИЧЕСКИ
            ow = {
                ...
                i.guild.me: discord.PermissionOverwrite(connect=True, view_channel=True, manage_channels=True, move_members=True) # <-- БОТ
            }
...
class ContestJoinView(discord.ui.View):
    ...
    async def join(self, i, b):
        # В реальном боте тут запись в БД, пока просто визуал
        await i.response.send_message("✅ Вы записаны!", ephemeral=True)
...
class SocialsModal(discord.ui.Modal, title="Настройки"):
    yt_id = discord.ui.TextInput(label="YouTube Channel ID (для авто-поста)", placeholder="UCxxxxxxxxxxxx", required=False)
    async def on_submit(self, i): 
        cursor.execute("UPDATE configs SET social_yt=? WHERE guild_id=?", (self.yt_id.value, i.guild.id)); conn.commit()
...
# --- СИСТЕМА ОБНОВЛЕНИЯ (ЮТУБ + СТАТЫ) ---
@tasks.loop(minutes=5)
async def tasks_loop():
    cursor.execute("SELECT guild_id, stats_category_id, social_yt, media_channel_id, last_video_id FROM configs")
    for row in cursor.fetchall():
        ...
        # 2. YouTube Автопост
        if row[2] and row[3]: # social_yt (Channel ID) and media_channel_id
            try:
                feed = feedparser.parse(f"https://www.youtube.com/feeds/videos.xml?channel_id={row[2]}")
                if feed.entries:
                    latest = feed.entries[0]
                    if latest.yt_videoid != row[4]: # Если новое видео
