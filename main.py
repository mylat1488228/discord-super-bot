...
# --- СИСТЕМА ПРАВ (ИЗОЛЯЦИЯ) ---
def get_perms_newbie(guild):
    # Видят только новички (@everyone)
    return {
...
def get_perms_verified(guild):
    # Видят ТОЛЬКО верифицированные
    c = get_config(guild.id)
    ver_role = guild.get_role(c[1]) if c and c[1] else None
    
    overwrites = {
        guild.default_role: discord.PermissionOverwrite(view_channel=False), # Скрыто для новичков
        guild.me: discord.PermissionOverwrite(view_channel=True, read_messages=True, send_messages=True)
    }
    if ver_role:
        overwrites[ver_role] = discord.PermissionOverwrite(
            view_channel=True,
            read_messages=True, 
            read_message_history=True, 
            send_messages=False # По умолчанию писать нельзя (в меню)
        )
    return overwrites

def get_perms_admin(guild):
    # Скрыто вообще для всех, кроме админов
    return {
...
class PrivateVoiceCreateModal(discord.ui.Modal, title='Создать комнату'):
    ...
    async def on_submit(self, i):
        try:
            ...
            # Бот дает себе полные права, а Verified - только если создатель пустит (или если открыто)
            ow = {
...
class SocialsModal(discord.ui.Modal, title="Настройки"):
    ...
    async def on_submit(self, i): cursor.execute("UPDATE configs SET social_yt_id=? WHERE guild_id=?", (self.yt_id.value, i.guild.id)); conn.commit(); await i.response.send_message("✅", ephemeral=True)

class AdminSelect(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    
    @discord.ui.button(label="🎵 Создать Музыку", style=discord.ButtonStyle.blurple, row=0)
    async def b_mc(self, i, b): ch=await i.guild.create_text_channel("🎶┃music", overwrites=get_perms_verified(i.guild)); update_config(i.guild.id, "music_text_channel_id", ch.id); await i.response.send_message(f"✅ {ch.mention}", ephemeral=True)
    
    @discord.ui.button(label="📺 Создать Медиа", style=discord.ButtonStyle.danger, row=0)
    async def b_md(self, i, b): ch=await i.guild.create_text_channel("📺┃медиа", overwrites=get_perms_verified(i.guild)); update_config(i.guild.id, "media_channel_id", ch.id); await i.response.send_message(f"✅ {ch.mention}", ephemeral=True)

    @discord.ui.button(label="🎉 Создать Конкурсы", style=discord.ButtonStyle.success, row=0)
    async def b_cn(self, i, b): ch=await i.guild.create_text_channel("🎉┃конкурсы", overwrites=get_perms_verified(i.guild)); update_config(i.guild.id, "contest_channel_id", ch.id); await i.response.send_message(f"✅ {ch.mention}", ephemeral=True)
    ...
    async def b_pv(self, i, b):
        ...
        ch=await i.guild.create_text_channel("create-room", overwrites=get_perms_verified(i.guild)); await ch.send(embed=discord.Embed(title="🔊 Личный Войс", color=discord.Color.fuchsia()), view=PrivateVoiceView()); await i.response.send_message("✅", ephemeral=True)
    
    @discord.ui.button(label="🏪 Меню Рынка", style=discord.ButtonStyle.secondary, row=1)
    async def b_mm(self, i, b): ch=await i.guild.create_text_channel("create-ad", overwrites=get_perms_verified(i.guild)); await ch.send(embed=discord.Embed(title="🏪 Рынок", color=discord.Color.orange()), view=MarketSelectView()); await i.response.send_message("✅", ephemeral=True)
    ...
    async def b_mk(self, i, b):
        ...
        ft = await i.guild.create_text_channel("🧡┃рынок-ft", category=cat, overwrites=get_perms_verified(i.guild))
        hw = await i.guild.create_text_channel("💙┃рынок-hw", category=cat, overwrites=get_perms_verified(i.guild))
    ...
    async def b_t(self, i, b): 
        ...
        l=await i.guild.create_text_channel("ticket-logs", category=c, overwrites=get_perms_admin(i.guild)); update_config(i.guild.id, "ticket_log_channel_id", l.id)
        update_config(i.guild.id, "ticket_category_id", c.id); ch=await i.guild.create_text_channel("tickets", category=c, overwrites=get_perms_verified(i.guild)); await ch.send(embed=discord.Embed(title="Тикеты"), view=TicketStartView()); await i.response.send_message("✅", ephemeral=True)

    @discord.ui.button(label="🛠 Верификация", style=discord.ButtonStyle.green, row=3)
    async def b_v(self, i, b):
        # 1. Создаем роль Verified
        ...
        # 2. Создаем канал Verify (Виден новичкам)
        ch=await i.guild.create_text_channel("verify", overwrites=get_perms_newbie(i.guild))
        await ch.send(embed=discord.Embed(title="Верификация", view=VerifyView())); await i.response.send_message("✅", ephemeral=True)
        
        # 3. Закрываем сервер для @everyone (Изоляция)
...
class TicketStartView(discord.ui.View):
    ...
    async def c(self, i, b):
        ...
        # Права тикета: Автор + Поддержка (c[2])
        ...
        ch=await i.guild.create_text_channel(f"ticket-{random.randint(1,999)}", category=cat, overwrites=ow)
