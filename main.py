...
# Создаем все необходимые колонки
...
# --- ГЕНЕРАТОР КАРТИНОК (Souls Visuals) ---
...
class PrivateVoiceControl(discord.ui.View):
    ...
    @discord.ui.select(cls=discord.ui.UserSelect, placeholder="➕ Выбрать пользователей (Whitelist)", min_values=1, max_values=10, row=0)
    async def whitelist_user(self, interaction: discord.Interaction, select: discord.ui.UserSelect):
        # Проверка, владелец ли нажал
        cursor.execute("SELECT owner_id FROM voice_channels WHERE voice_id = ?", (self.voice_channel.id,))
        res = cursor.fetchone()
        if not res or res[0] != interaction.user.id:
            return await interaction.response.send_message("❌ Вы не владелец комнаты.", ephemeral=True)

        for user in select.values:
            await self.voice_channel.set_permissions(user, connect=True, view_channel=True)
        await interaction.response.send_message(f"✅ Доступ выдан: {', '.join([u.name for u in select.values])}", ephemeral=True)

    @discord.ui.button(label="🔒 Закрыть от всех", style=discord.ButtonStyle.red, row=1)
    async def lock_room(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.voice_channel.set_permissions(interaction.guild.default_role, connect=False, view_channel=False)
        await interaction.response.send_message("🔒 Комната закрыта. Добавляйте людей через меню выше.", ephemeral=True)

    @discord.ui.button(label="🔓 Открыть для всех", style=discord.ButtonStyle.green, row=1)
...
class PrivateVoiceCreateModal(discord.ui.Modal, title='Создание комнаты'):
    v_name = discord.ui.TextInput(label='Название', placeholder='Моя база', max_length=20)
    v_limit = discord.ui.TextInput(label='Лимит людей (число)', placeholder='0 = безлимит', max_length=2, required=False)

    async def on_submit(self, interaction: discord.Interaction):
        conf = get_config(interaction.guild.id)
        # conf[14] = pvoice_category_id
        if not conf or not conf[14]: return await interaction.response.send_message("❌ Категория приваток не настроена.", ephemeral=True)
        
        category = interaction.guild.get_channel(conf[14])
        try: limit = int(self.v_limit.value) if self.v_limit.value else 0
        except: limit = 0
        
        # Права: Видят все, зайти не может никто (пока не создана)
        overwrites = {
        ...
        cursor.execute("INSERT INTO voice_channels (voice_id, owner_id) VALUES (?, ?)", (vc.id, interaction.user.id))
        conn.commit()
        
        # Перекидываем, если он в войсе
        if interaction.user.voice:
            await interaction.user.move_to(vc)
        
        # Отправляем ПАНЕЛЬ УПРАВЛЕНИЯ (скрытую)
        embed = discord.Embed(title=f"⚙️ Управление: {self.v_name.value}", description="Используйте меню ниже, чтобы пускать людей в закрытую комнату.", color=discord.Color.gold())
        await interaction.response.send_message(embed=embed, view=PrivateVoiceControl(vc), ephemeral=True)
...
class PrivateVoiceView(discord.ui.View):
    ...
    async def create_voice(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(PrivateVoiceCreateModal())

# --- 2. СТАТИСТИКА (ЗАКРЫТЫЙ КАНАЛ) ---
...
class MarketModal(discord.ui.Modal, title='Продажа'):
    ...
    async def on_submit(self, i):
        ...
        emb.add_field(name="📝", value=self.item_desc.value, inline=False)
...
class MarketSelectView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label="Продать на FunTime", style=discord.ButtonStyle.danger, emoji="🧡", custom_id="m_ft")
    ...
    @discord.ui.button(label="Продать на HolyWorld", style=discord.ButtonStyle.primary, emoji="💙", custom_id="m_hw")
...
class AdminSelect(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    # Row 0 - Каналы Рынка
    @discord.ui.select(cls=discord.ui.ChannelSelect, placeholder="Канал РЫНОК FT", channel_types=[discord.ChannelType.text], row=0)
    async def s_ft(self, i, s): update_config(i.guild.id, "market_ft_channel_id", s.values[0].id); await i.response.send_message("✅ FT", ephemeral=True)
    
    # Row 1 - Каналы Приветствий/Музыки
    @discord.ui.select(cls=discord.ui.ChannelSelect, placeholder="Канал Музыки", channel_types=[discord.ChannelType.text], row=1)
    async def s_mus(self, i, s): update_config(i.guild.id, "music_channel_id", s.values[0].id); await i.response.send_message("✅ Музыка", ephemeral=True)
    @discord.ui.select(cls=discord.ui.ChannelSelect, placeholder="Канал РЫНОК HW", channel_types=[discord.ChannelType.text], row=1)
    async def s_hw(self, i, s): update_config(i.guild.id, "market_hw_channel_id", s.values[0].id); await i.response.send_message("✅ HW", ephemeral=True)
    ...
    async def btn_stat(self, i, b):
        ...
        await i.response.send_message(f"✅ Стат-канал создан", ephemeral=True)

    @discord.ui.button(label="🔊 Приватки", style=discord.ButtonStyle.blurple, row=2)
    async def btn_pv(self, i, b):
        ...
        await i.response.send_message(f"✅ Меню приваток создано", ephemeral=True)

    @discord.ui.button(label="🏪 Рынок", style=discord.ButtonStyle.danger, row=2)
    async def btn_m(self, i, b): ch=await i.guild.create_text_channel("create-ad"); await ch.send(embed=discord.Embed(title="🏪 Рынок", color=discord.Color.orange()), view=MarketSelectView()); await i.response.send_message("✅", ephemeral=True)
    ...
    async def btn_v(self, i, b):
        ...
        try: await i.guild.default_role.edit(permissions=discord.Permissions(read_messages=False)); await i.response.send_message("✅", ephemeral=True)
        except: await i.response.send_message("✅ (Скройте каналы вручную)", ephemeral=True)

    @discord.ui.button(label="🎫 Тикеты", style=discord.ButtonStyle.gray, row=3)
    async def btn_t(self, i, b): c=await i.guild.create_category("Support"); update_config(i.guild.id, "ticket_category_id", c.id); ch=await i.guild.create_text_channel("tickets", category=c); await ch.send(embed=discord.Embed(title="Тикеты"), view=TicketStartView()); await i.response.send_message("✅", ephemeral=True)

# --- СТАРЫЕ VIEW (Verify, Tickets) ---
...
@app_commands.describe(market="ft / hw", price="Цена", item="Товар", photo="Фото (файл)")
async def slash_sell(interaction: discord.Interaction, market: str, price: str, item: str, photo: discord.Attachment = None):
    ...
    await interaction.response.send_message(f"✅ Отправлено", ephemeral=True)

@bot.tree.command(name="online", description="Онлайн FunTime")
async def slash_online(interaction: discord.Interaction):
    try:
        s = await JavaServer.async_lookup(FUNTIME_IP); st = await s.async_status()
        await interaction.response.send_message(embed=discord.Embed(title="FunTime", description=f"Онлайн: {st.players.online}", color=discord.Color.orange()))
...
async def setup(ctx):
    if ctx.author.guild_permissions.administrator or ctx.author.name in ADMINS:
        overwrites = {ctx.guild.default_role: discord.PermissionOverwrite(read_messages=False), ctx.guild.me: discord.PermissionOverwrite(read_messages=True), ctx.author: discord.PermissionOverwrite(read_messages=True)}
...
async def on_member_join(member):
    ...
    # Авто-сетап при добавлении бота
    if member.id == bot.user.id:
        try:
            ow = {member.guild.default_role: discord.PermissionOverwrite(read_messages=False), member.guild.me: discord.PermissionOverwrite(read_messages=True)}
            cat = await member.guild.create_category("BOT SETTINGS", overwrites=ow)
            ch = await member.guild.create_text_channel("admin-panel", category=cat)
            await ch.send(embed=discord.Embed(title="⚙️ Админ Панель"), view=AdminSelect())
        except: pass
